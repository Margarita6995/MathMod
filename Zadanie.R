library(readr)
dt <- read_csv("C:/Artem/HP/Downloads/eddypro.csv", 
...
mod3 = lm(co2_flux ~ (H + co2_flux + co2_mole_fraction + co2_mixing_ratio)^2, data=dt)
summary(mod3)
#построение графиков модели № 3
plot(mod3)
#построение модели № 4 со взаимодействиями оставшихся переменных
mod4 = lm(co2_flux ~ (H + co2_flux + co2_mole_fraction + co2_mixing_ratio)^2 -H:co2_mole_fraction, data=dt)
summary(mod4)
